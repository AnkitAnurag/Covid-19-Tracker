...
const FetchCountryStats = () => {
  ...
  if (loading) {
    return (
      <div style={{color: `${currentTheme.textColor}`}}>
        ...
        <div className="d-flex justify-content-center" style={{marginTop:"250px"}}>
          ...
          <div className="spinner-border" role="status" style={{width:"40px",height:"40px"}}></div>
        </div>
    ...
    const fatality = (cname[country].TotalDeaths/cname[country].TotalConfirmed)*100;
    const fatalityPercent = fatality.toFixed(2);
    const recovery = (cname[country].TotalRecovered/cname[country].TotalConfirmed)*100;
    const recoveryPercent = recovery.toFixed(2);
    const str = stats.Date;
    var date = str.substring(0, 10);
    var time = str.substring(11, 19);
    var activeCases = (cname[country].TotalConfirmed-cname[country].TotalDeaths-cname[country].TotalRecovered);
    return(
      <div>
        <div className="input-group mx-auto mt-5" style={{justifyContent:"center"}}>
            <form className="form-inline">
                <div className="input-group-prepend">
                    <label className="input-group-text">Select Country</label>
                </div>
                <select onChange={countrySelected} className="custom-select" id="inputGroupSelect01">
